...
let copyBLOCKS: Blocks[] = clonedeep(BLOCKS);
...
function isRotate(array: Array<number>) {
  ...
  if(!leftWall && !rightWall || !isFilled) {  
    ...
    copyBLOCKS[controllableBlock].number = blockRotatableArray();
    return true;
  }
}
...
function isBlocksGathersInRow() {
  if(//left === false && right === false && 
    ...
    oneRowArray.forEach((v)=>{
      ...
      const checkROWs = v.every(item => item !== 'empty');
      if(checkROWs) {
        console.log('complete!!!!!!');
        return isIncludeEmpty = true;
      }
    })
...
function draw() {
  ...
  field.forEach((v,i)=>{
    ...
    if( v === 'current') {
      ...
      ctx.fillRect((fixToFirstDigit(i)*30)+1, (Math.floor(i/10)*30)+1, 28, 28);
      ctx.fill();
    // draw fixed block
    }
    if (v !== 'current' && typeof v === 'number') {
...
window.addEventListener('keydown', event => {
 ...
 if(down === false) {
  ...
  //   if(State.Movable.rotate) {
  //     State.Block.angle = Data.NUMBER.DEGREES;
  //     State.Block.current = State.rotatedBlock(State.Block.current);
  //   }
  // }
